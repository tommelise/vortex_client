...
def get_person(url,new_doc,rel_dest_url,name)
  if( URI.decode(name) =~ /<h2>([^<]*)/)
    ...
    if get_user(url,name,old_link)
      ...
      @log = @log + "Name: #{full_name}, user_id: #{user_id}<br/>\nOld presentation: #{old_link} => " + new_link + "</p>\n\n"
      @position = ""
      @phone = ""
      @email = ""
      description = " "
...
def get_user_id(full_name)
  ...
  if (ldap_user_id) && full_name !="Merethe Andersen"
...
puts description
  ...
  url = rel_dest_url + @group_name + "/" + username + "/index.html"  
 ...
 if(username.include?("-"))
    getExternalPersonInfo = "false"
    data = JSON.parse(@vortex.get(url))
    data['properties']['username'] = " "
    data['properties']['firstName'] = firstname
    data['properties']['surname'] = surname
    data['properties']['getExternalPersonInfo'] = getExternalPersonInfo
    data['properties']['position'] = @position
    data['properties']['phone'] = @phone
    data['properties']['email'] = @email
    
    @vortex.put_string(url, data.to_json)
  end
...
test_doc = ""
#get_person(url,test_doc, rel_dest_url, '<a href="http://folk.uio.no/geirs/" target="_blank" class="vrtx-link-check">Storvik, Geir</a>')
# @group_name="phdstudents"
#get_person(url,test_doc, rel_dest_url, '<a href="http://www.cees.uio.no/news/new-faces/irene-parra.html">Parra, Irene</a>')
